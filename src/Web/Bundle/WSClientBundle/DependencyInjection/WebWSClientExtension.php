...
namespace Web\Bundle\WSClientBundle\DependencyInjection;
 ...
 */
class WebWSClientExtension extends Extension
{
    /**
     * {@inheritDoc}
     */
    ...
    {
        $serviceId  = ‘web_ws_client' . ($alias != 'default' ? '_' . $alias : '');
        $definition = new Definition('Web\Bundle\WSClientBundle\Adapter\WSClientAdapterInterface');
        $definition->setFactoryService(‘web_ws_client.factory');
        ...
        if (array_key_exists('cache', $config)) {
            ...
            if ($config['cache']['force_request_ttl']) {
                ...
                $canCacheClass = ['\Web\Bundle\WSClientBundle\Cache\Guzzle\ForcedCacheRequest', 'canCacheRequest'];
            } else {
